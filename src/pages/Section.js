                ...
                <div className="flex gap-y-2 h-5/6 w-11/12 flex-col items-center rounded-lg bg-blue-400 p-5">
                    ...
                    <div className="mt-5 flex h-5/6 w-full flex-col items-center justify-center overflow-auto">
                        ...
                        <div className="w-full self-start flex justify-between items-center text-md font-bold py-2">
                            ...
                            <div className="p-2 rounded-lg invisible sm:visible bg-red-500 hover:bg-red-600 duration-200" onClick={()=> navigate('/teacher')}>
                            ...
                            {students && accounts.map((account) => {
                                if (searchTerm === ""){
                                    ...
                                    if(students.includes(account.username)){
                                        return(
                                            <div key={account.username} className="flex items-center justify-center rounded-lg bg-green-500 hover:bg-green-400 duration-200 p-5 mb-2 text-sm sm:text-base cursor-pointer" onClick={() => {setIsOpen3(true);localStorage.setItem("student_id", account.username.toLowerCase());localStorage.setItem("student_name", account.name.toLowerCase())}}>
                    ...
                    <div className="w-full grid grid-cols-2 sm:grid-cols-5 gap-1 m-1">
                        <button className="font-bold break-words text-sm sm:text-xl p-2 bg-green-500 hover:bg-green-600 duration-200 rounded-lg" onClick={() => setIsOpen(true)}>Enroll student</button>
                        <button className="font-bold break-words text-sm sm:text-xl p-2 bg-green-500 hover:bg-green-600 duration-200 rounded-lg" onClick={() => setIsOpen2(true)}>Rankings</button>
                        <button className="font-bold break-words text-sm sm:text-xl p-2 bg-green-500 hover:bg-green-600 duration-200 rounded-lg" onClick={() => setIsOpen4(true)}>Grading Structure</button>
                        <button className="font-bold break-words text-sm sm:text-xl p-2 bg-green-500 hover:bg-green-600 duration-200 rounded-lg" onClick={() => setIsOpen5(true)}>Activities</button>
                        <button className="font-bold break-words text-sm sm:text-xl p-2 bg-green-500 hover:bg-green-600 duration-200 rounded-lg" onClick={() => setIsOpen6(true)}>Grades</button>
                    </div>
                </div>
            </div>
